...
| CT-TRF-07 | Conta de destino inexistente, mas em formato numérico válido | Particionamento de equivalência | Saldo de origem: 1.000,00 · Valor: 100,00 · Conta de destino: número numérico que não corresponde a nenhuma conta cadastrada · Descrição preenchida | Regra R4 → **Reprovado** — "Conta inválida ou inexistente" | Alta |
...
| CT-TRF-09 | Campo "Descrição" vazio | Particionamento de equivalência (classe inválida) | Saldo de origem: 1.000,00 · Valor: 100,00 · Conta de destino válida · Descrição: *(vazio)* | Viola RN-05 → **Reprovado**, campo obrigatório | Alta |
...
- Diferente do artefato do ParaBank, aqui não houve leitura de código-fonte: as regras vieram integralmente da especificação oficial publicada pelo próprio BugBank. Isso torna o levantamento mais rápido e diretamente rastreável a uma fonte pública, mas significa que nenhum dos casos acima foi confirmado por execução real ainda — todos os resultados esperados estão baseados na leitura literal da especificação, não em comportamento observado.
- Vários casos (CT-TRF-08, CT-TRF-10, CT-TRF-11, CT-TRF-12, CT-TRF-14) têm resultado esperado sinalizado explicitamente como "a confirmar", porque a especificação não é exaustiva o bastante para prevê-los com certeza — são justamente os melhores candidatos para a próxima sessão de teste exploratório.
